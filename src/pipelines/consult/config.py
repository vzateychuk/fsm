...
class PatientInfo:
    ...
    Loaded from config/patient.yaml. Used to populate Patient Info section
    in the consultation prompt.
    """
...
class RetrievalUsageConfig:
    ...
    These parameters control HOW the retrieval system is used within a consultation,
    not how the retrieval system itself works. The actual retrieval system configuration
    (BM25 weights, prefix matching, etc.) is stored in RetrievalConfig from config/retrieve.yaml.
    """

    query_top_k: int
    """Maximum number of chunks to fetch from BM25 query for the consultation.
    Passed as 'limit' to RetrieveRequest."""

    query_limit_per_document: int
    """Maximum chunks per document in the BM25 query results (diversity cap).
    Passed as 'limit_per_document' to RetrieveRequest."""
...
class RecencyConfig:
    ...
    the LLM has access to the most up-to-date information. These parameters
    control which documents are included and how many chunks per document.
    """
    ...
    db_fetch_limit: int
    """Fetch limit for database query before date filtering on client side.
    Should be >= max_docs with some buffer to account for excluded old documents."""
...
class BundleConfig:
    ...
    The bundle aggregates query chunks (from BM25) and recency chunks into a single
    context block with enforced limits on quantity and size. These parameters ensure
    the final prompt fits within context limits and model constraints.
    """

    max_total_chunks: int
    """Hard limit on total chunks in the final bundle (query + recency combined).
    After this limit is applied, chunks are truncated by line count and then by
    character count."""
...
class ExcerptsConfig:
    ...
    Text excerpts are truncated based on their category to save context while
    preserving the most informative content. These parameters control which
    categories get full text vs. truncated text, and how aggressively to truncate.
    """
...
class ConsultConfig:
    ...
    LLM authentication settings are passed via environment variables.
    ...
    def load(cls, config_path: Path) -> ConsultConfig:
        ...
        Returns:
            ConsultConfig instance with all retrieval usage, recency, bundle, and excerpts settings.

        Note:
            Retrieval system parameters are loaded separately from config/retrieve.yaml via RetrievalConfig.
            LLM parameters (base_url, api_key, model) are passed via environment
            variables to keep secrets separate.
        """
        with open(config_path, encoding="utf-8") as f:
            data = yaml.safe_load(f)
