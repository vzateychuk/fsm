...
- `documents`: `id`, `source_path`, `doc_type`, `source_sha256`, `indexed_at`
...
Retrieval по умолчанию **не загружает** документ целиком — возвращает `source_path`. Подгрузка выполняется оркестратором по необходимости (управляется параметрами запроса: `include_full_docs`, `context_window`).

Предусловие: `documents.source_path` должен быть ключом FileStore (например `.data/filestore/consultation/abc123.md`), а не исходным путём к файлу. Это обязан гарантировать ingest шаг S8 PersistSourceFile + S9 PersistDocument: `source_path` записывается из `ctx.data.filestore_path`, а не из `ctx.input.source_path`. Если ingest пишет исходный путь, `include_full_docs=True` будет нестабильным.
...
- `doc_type_hint: DocType | None` — подсказка для soft/hard фильтрации по типу документа.
...
- `documents: list[DocumentEvidence]` — группировка по документам:
  - `document_id`, `source_path`, `doc_type`, `chunks: list[ChunkSearchResult]`
...
| `doc_type` | RetrieveRequest / intent | SQL-фильтр по типу (или None) |
...
### Шаг R2. Intent detection (эвристика)

Определить тип запроса без LLM:
- ключевые слова ("анализы", "показатели") → `doc_type_hint = lab`
- ("узи", "мрт", "фгдс", "рентген") → `doc_type_hint = diagnostic`
- ("рекомендации", "назначения", "диагноз") → `doc_type_hint = consultation`

Результат: `IntentInfo` (detected_type, confidence, matched_keywords).

`detected_type` — только подсказка, не жёсткий фильтр. При `RETRIEVE_DOC_TYPE_MODE=soft` (default) он не добавляется ни в WHERE-условие, ни в boost — только в debug-поле. Причина: диагностические процедуры (УЗИ, МРТ, ФГДС) часто упоминаются в документах типа `consultation`, поэтому intent → doc_type ненадёжен для жёсткой фильтрации.
...
WHERE chunks_fts MATCH ?
  [AND d.doc_type = ?]              -- только при RETRIEVE_DOC_TYPE_MODE = 'hard'
...
Опциональная doc_type фильтрация определяется настройкой `RETRIEVE_DOC_TYPE_MODE`:
- `"soft"` (default): не фильтровать и не применять boost (нет WHERE-фильтра по doc_type, нет переранжирования в MVP).
- `"hard"`: применять `WHERE doc_type = ?` если intent уверен.

`KnowledgeStore.search_chunks()` принимает:
- `query` = `fts_match` (FTS5 MATCH DSL строка из QueryPlan)
- SQL-фильтры: `doc_type`, `document_id`, `kinds`, `section_path_prefix`
- Параметры выдачи: `limit`, `prelimit`, `limit_per_document`

Store не выполняет нормализацию, alias-expansion и prefix rules — это делает retrieval pipeline до вызова store.

Внимание: текущая реализация `search_chunks()` использует `bm25(chunks_fts)` без весов. Ранжирование не соответствует таблице выше до замены на `bm25(chunks_fts, 1.0, 2.5, 2.0, 3.5)`.
...
Примечание о soft mode: `RETRIEVE_DOC_TYPE_MODE=soft` означает "не добавлять WHERE-фильтр по doc_type". Это не boost — никакого переранжирования в MVP нет. Soft = нет фильтра и нет boost. Hard = WHERE-фильтр.
...
- `context_window > 0`: для каждого выбранного чанка загрузить соседние чанки (`chunk_no ± context_window`) из той же таблицы `chunks` через `get_chunks_for_document(document_id, from_no, to_no)`. В MVP можно не реализовывать.
- `include_full_docs = True`: загрузить полный markdown по `source_path` из FileStore. Применяется, когда оркестратор запрашивает полный контекст документа.
...
- `documents: list[DocumentEvidence]` — каждый содержит `document_id`, `source_path`, `doc_type`, вложенные chunks
...
| `FILESTORE_ROOT` | `.data/filestore` | Корень FileStore для optional enrich |
...
| `RETRIEVE_DOC_TYPE_MODE` | `"soft"` | `soft` — нет фильтра и нет boost; `hard` — WHERE-фильтр |
