...
| `document_id` | S8 PersistDocument | `str` | не `None`; детерминирован от `file_hash` |
| `chunk_ids` | S9 PersistChunks | `list[str]` | `len == len(tagged_chunks)` |
| `fts_updated` | S10 UpdateFTS | `bool` | всегда `True` после шага |
...
|---|---|---|
| S8 PersistDocument | `document_id` (= `file_hash[:32]`) | `INSERT OR REPLACE` |
| S9 PersistChunks | `document_id` | `DELETE WHERE document_id + INSERT` |
| S10 UpdateFTS | `document_id` | `DELETE + INSERT` |
...
## Phase 8: SQLite persistence (S8–S10 + SqlStore)

**Цель:** `PersistDocument`, `PersistChunks`, `UpdateFTS` записывают в реальную SQLite БД; `SqlStore` сохраняет checkpoint саги.
...
- `documents(id, source_path, source_sha256, doc_type, raw_text, indexed_at)`
- `chunks(id, document_id, chunk_no, section_path, heading, kind, text, tags_text)`
- `chunks_fts` как FTS5 content table (`content='chunks'`, поля: `text`, `heading`, `section_path`, `tags_text`)
...
**8.3 `PersistDocument`**
Upsert в `documents`: `INSERT OR REPLACE`; `document_id = sha256[:32]`.
*Результат:* `SELECT * FROM documents` возвращает 1 строку с корректными `source_path`, `doc_type`, `source_sha256`.

**8.4 `PersistChunks`**
`DELETE FROM chunks WHERE document_id = ?` + batch INSERT; `chunk_id` — детерминированный хэш от `doc_id + section_path + kind + text`; присвоить `chunk_no`.
*Результат:* `SELECT COUNT(*) FROM chunks` совпадает с `len(ctx.data.chunk_ids)`.

**8.5 `UpdateFTS`**
Explicit sync: DELETE из `chunks_fts` по `doc_id` + INSERT из `chunks` (rowid, text, heading, section_path, tags_text). Стратегия синхронизации: явный reindex по `document_id` при каждом запуске S10; инкрементальная синхронизация и триггеры не используются на MVP.
*Результат:* `SELECT text FROM chunks_fts WHERE chunks_fts MATCH 'введение'` возвращает релевантные чанки.
...
| S8 | PersistDocument | `document_id`, запись в `documents` |
| S9 | PersistChunks | `chunk_ids`, записи в `chunks` |
| S10 | UpdateFTS | `fts_updated=True`, синхронизация `chunks_fts` |
